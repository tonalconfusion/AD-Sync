...
write-host ("--------------------------------------------------------------------------------------------------------") -ForegroundColor Red
sleep(1)
New-variable -name CSVFile -Value  "C:\scripts\SyncADUsersCsv\Users-dev.csv" -Visibility Private
}
Else {
New-variable -name CSVFile -Value  "C:\scripts\SyncADUsersCsv\Users.csv" -Visibility Private
}
New-variable -name CSVExportFile -Value  "C:\scripts\SyncADUsersCsv\Users-Export.csv" -Visibility Private
...
New-variable -name Log  -value "C:\scripts\logs\AdSync-Log-$Today_Date-V$myVer.txt" -Visibility Private
$CompanyName = "DKPCREPAIRS"
$distinguishedName = "DC=dkpcrepairs,DC=local"
$UPND = "@dkpcrepairs.local"
...
$CSVImport = (read-sqltabledata -serverInstance localhost -databasename ADsync -schemaname dbo -tablename Users) | sort EmployeeID | sort Status -Descending
...
ForEach ($entry in $CSVImport)
    {
        ...
        $UpdateDateQuery = "UPDATE       Users","SET                Updated = '$updatedate'","WHERE        (samAccountName = '$updateSamAcctName')"
        invoke-sqlcmd -ServerInstance localhost -Query "$UpdateDateQuery" -Database ADSYNC 

    }

#write-host "Sleep2"
#Sleep(1.5)
#Remove-Item -path $CSVExportFile -force
#Add-Content -Path $CSVExportFile `
#-Value "Status,GivenName,Surname,samAccountName,DisplayName,Street,State,City,PostCode,Country,Manager,EmployeeID,Company,Description,Department,Updated,ScriptPath,Password,OfficeNumber,MobileNumber"
...
#ForEach ($entry in $CSVImport)
 #   {
  #       Add-content -path $CSVExportFile -Value (`
  #       $entry.Status + "," +`
  #       $entry.GivenName + "," +`
  #       $entry.Surname + "," +`
  #       $entry.samAccountName + "," +`
...
#write-host "Sleep Before Cleanup"
#Sleep(.1)

# Write-SqlTableData -ServerInstance localhost -InputData $CSVImport -DatabaseName ADsync -SchemaName dbo -TableName Users

...
DA-1.5.3  03-03-18
          Spawned a Domain And Services Script
...
Function Write-Log($Info){
    ...
    $Date = Get-Date -Format "d/M/yyyy hh:mm:ss"
        ...
        Else {
            $INPUTDATA = New-Object PSObject -Property @{DateTime=$date;Entry=$info}
            Write-SqlTableData -ServerInstance localhost -InputData $INPUTDATA -DatabaseName ADsync -SchemaName dbo -TableName LOGDA
            }
       #sleep(.1)              
      ...
      if($_.Status -notlike "*DELETED*") {
        ...
        if ($Password -le 7 ){$_.password = Get-ComplexPassword; Write-log ("Generic Password Created")} 
                if ($_.Status -eq "TRUE"){
                        ...
                        Write-Log ("Adding "+ $_.SamaccountName + " to Domain Admins")
    
                        # Checking if Domain Admins
                        if ($Companyname -eq "Domain Admins"){
                        Write-Log ("Adding "+ $_.SamaccountName + " to Domain Admins")
                        ...
                        elseif ($_.Status -eq "FALSE") {
                            ...
                            Write-Log ("Removing "+ $_.SamaccountName + " from Domain Admins")
                            remove-ADGroupMember -identity "Domain Admins" -Members $_.SamAccountName -confirm:$false
            ...
            if (($_.Status -eq "TRUE") -and ($_.Department -notlike "*Service*"))
                {
                 ...
                 if ($user.DistinguishedName -notlike "*$baseou*")
                        ...
                        if ($Companyname -eq "Domain Admins"){
                        Write-Log ("Adding "+ $_.SamaccountName + " to Domain Admins")
                        add-ADGroupMember -identity "Domain Admins" -Members $_.SamAccountName -confirm:$false
                                                               }
                        }
                }
